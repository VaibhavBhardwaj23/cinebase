...
export const Header = () => {
  ...
  const [darkMode, setDarkMode] = useState(
    JSON.parse(localStorage.getItem("cineBaseDark")) || false
  );
  const active =
  ...
  useEffect(() => {
    localStorage.setItem("cineBaseDark", JSON.stringify(darkMode));
    if (darkMode) {
    ...
    <nav className="bg-white border-b-2 border-gray-200 dark:bg-gray-900 dark:border-gray-900">
      ...
      >
        <Link to="/" className="flex items-center">
          <img src={Logo} className="h-8 mr-3" alt="CineBaseLogo" />
        ...
        <div className="flex md:order-2">
          ...
          <div id="mobile-nav" className="relative hidden md:block">
